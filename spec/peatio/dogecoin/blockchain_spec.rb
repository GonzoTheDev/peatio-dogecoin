RSpec.describe Peatio::Dogecoin::Blockchain do
  ...
  context :configure do
    ...
    it 'currencies and server configuration' do
      currencies = [{ id: :doge,
                      ...
                      options: {} }]
      settings = { server: 'http://user:password@127.0.0.1:44555',
  ...
  context :latest_block_number do
    ...
    let(:server) { 'http://user:password@127.0.0.1:44555' }
    let(:server_without_authority) { 'http://127.0.0.1:44555' }
    ...
    it 'raises error if there is error in response body' do
      stub_request(:post, 'http://127.0.0.1:44555')
        .with(body: { jsonrpc: '1.0',
                      method: :getblockcount,
                      params:  [] }.to_json)
